...
## This function takes an object class phylo, two parameters (root.state and variance) and trait values (data) 
## and outputs log-likelihood under a Brownian motion model 
## Note: under the BM model, the trait evolution over time can be modelled as 
## E[z(t)] ~ N(z(0),sigma^2*t1) where z(0) is the root state, t is branch length and sigma^2 is rate parameter (diffusion rate)
...
# use wingL as trait values (data)
wingL <- geodata$wingL
names(wingL) <- row.names(geodata)

# BM log-likelihood function 
BM_loglik <- function(tree, variance, root.state, data) {
  ...
  # Rate is a rate matrix representing rate of evolution 
  # multipling C by variance
  Rate <- C * variance; 
  # also convert inv.C to the inverse of the rate
  inv.Rate <- inv.C * variance ^-1; 
  # calculate the log-likelihood 
  # The BM likelihood calculation uses a standard formula for the likelihood of a multivariate normal distribution
...
# plot log-likelihood curve with a fixed value of root state
...
# plot likelihood curve with a fixed value of root state
...
# and outputs maximum log-likelihood and parameter estimates based on selected model
# find ML and parameter estimates under the BM model 
bm.ml <- fitContinuous(phy= geotree, wingL, model="BM")
# bm.ml, sigsq = 0.070546, z0 = 4.205953, log-likelihood = 8.243269
# assign these values to BM_loglik function
...
# and outputs the log-likelihood under BM model given the parameters  

Pagel_loglik <- function(tree, variance, root.state, data, model = NULL, transform.para) {
  # transforms branch length according to Pagel models 
...
# comparing Pagel_loglik with the available R function from the R package geiger
# lambda model
